 ...
 * 
 * 세션 기반 로그인 방식
 * 토큰 기반 로그인 방식 [v] - JsonWebToken
 * 
 * JWT 프로세스
 * 
 * 1. 사용자 인증(로그인)
 * 아이디 비밀번호를 입력해서 로그인 할 것 -> 성공하면 JWT를 만들어 줄 것 -> 응답데이터에 JWT 보내줄 것
 * 
 * 2. 클라이언트 입장(브라우저)
 ...
 * 유효성검사를 통과하면 클레임을 까봐서 권한체크를 하고 요청 처리한다. _ 유효성 검사를 통과 안될 시 필터를 만들어 에게 위임할 것
 * 
 **/
